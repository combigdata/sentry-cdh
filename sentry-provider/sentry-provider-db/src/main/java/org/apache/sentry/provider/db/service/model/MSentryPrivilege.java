...
public class MSentryPrivilege {
  ...
  public void appendPrincipal(PrivilegePrincipal principal) {
    if(principal.getType() == SentryPrincipalType.ROLE) {
      roles.add((MSentryRole)principal);
    } else  if(principal.getType() == SentryPrincipalType.USER) {
  ...
  public void removePrincipal(PrivilegePrincipal principal) {
    if(principal.getType() == SentryPrincipalType.ROLE && (roles != null) && (roles.size() > 0)) {
      roles.remove((MSentryRole)principal);
    } else  if(principal.getType() == SentryPrincipalType.USER && (users != null) && (users.size() > 0)) {
