...
/**
 * SentryStateBank is a state visibility manager to allow components to communicate state to other
 * parts of the application.
 *
 * It allows you to provide multiple boolean states for a component and expose those states to
 * other parts of the application without having references to the actual instances of the classes
 * setting those states.
 *
 * SentryStateBank uses a bitmasked long in order to store the states, so its very compact and
 * efficient.
 *
 * States are defined using an enum that implements the {@link SentryState} interface.  The
 ...
 * location to be checking for the state.
 *
 * The following is an example of a simple {@link SentryState} enum implementation
 * <pre>
 * {@code
 ...
 * </pre>
 *
 * This class is thread safe.  It uses a {@link ReentrantReadWriteLock} to wrap accesses and changes
 * to the state.
 */
@ThreadSafe
public final class SentryStateBank {
  ...
  public static boolean hasStatesEnabled(String component, Set<SentryState> passedStates) {
    ...
    try {
      ...
      if ((states.get(component) & value) == value) {
        return true;
      }
      return false;
    } finally {
