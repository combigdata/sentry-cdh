...
public class TestSentryShellIndexer extends SentryGenericServiceIntegrationBase {
  private static final Logger LOGGER = LoggerFactory.getLogger(TestSentryShellIndexer.class);
  ...
  public void testCreateDropRole() throws Exception {
    runTestAsSubject(new TestOperation() {
      ...
      public void runTestAsSubject() throws Exception {
        ...
        // validate the result
        Set<TSentryRole> roles = client.listAllRoles(requestorName, HBASEINDEXER);
  ...
  public void testAddDeleteRoleForGroup() throws Exception {
    runTestAsSubject(new TestOperation() {
      ...
      public void runTestAsSubject() throws Exception {
        ...
        client.createRole(requestorName, TEST_ROLE_NAME_1, HBASEINDEXER);
        client.createRole(requestorName, TEST_ROLE_NAME_2, HBASEINDEXER);
        ...
        // validate the result
        Set<TSentryRole> roles = client.listRolesByGroupName(requestorName, TEST_GROUP_1, HBASEINDEXER);
        assertEquals("Incorrect number of roles", 0, roles.size());
        roles = client.listRolesByGroupName(requestorName, TEST_GROUP_2, HBASEINDEXER);
        assertEquals("Incorrect number of roles", 0, roles.size());
        roles = client.listRolesByGroupName(requestorName, TEST_GROUP_3, HBASEINDEXER);
        assertEquals("Incorrect number of roles", 0, roles.size());
        // clear the test data
        client.dropRole(requestorName, TEST_ROLE_NAME_1, HBASEINDEXER);
        client.dropRole(requestorName, TEST_ROLE_NAME_2, HBASEINDEXER);
      }
    });
  }
      ...
      public void runTestAsSubject() throws Exception {
        ...
        for (int i = 0; i < privs.length; ++i) {
          ...
          Set<TSentryPrivilege> privileges = client.listPrivilegesByRoleName(requestorName,
            TEST_ROLE_NAME_1, HBASEINDEXER, service);
