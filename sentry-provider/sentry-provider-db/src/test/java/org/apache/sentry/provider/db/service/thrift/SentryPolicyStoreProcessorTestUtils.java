...
class SentryPolicyStoreProcessorTestUtils {
  ...
  SentryPolicyStoreProcessorTestUtils(Configuration conf, SentryStoreInterface sentryStore) throws Exception {
    ...
    this.processor = new SentryPolicyStoreProcessor(
      SentryPolicyStoreProcessor.SENTRY_POLICY_SERVICE_NAME, conf, sentryStore);
  }
  ...
  static TSentryPrivilege newPrivilegeOnDatabase(final String action, final String serverName,
    final String dbName) {
  ...
  static TSentryPrivilege newPrivilegeOnDatabaseWithGrant(final String action,
    final String serverName, final String dbName) {
    TSentryPrivilege privilege = new TSentryPrivilege(){{
      setPrivilegeScope(PrivilegeScope.DATABASE.toString());
      setServerName(serverName);
      setDbName(dbName);
  ...
  static TSentryPrivilege newPrivilegeOnTable(final String action, final String serverName,
  ...
  static TSentryPrivilege newPrivilegeOnTableWithGrant(final String action, final String serverName,
    final String dbName, final String tableName) {
    TSentryPrivilege privilege = new TSentryPrivilege(){{
      setPrivilegeScope(PrivilegeScope.TABLE.toString());
      setServerName(serverName);
  ...
  static TSentryPrivilege newPrivilegeOnColumn(final String action, final String serverName,
    final String dbName, final String tableName, final String column) {
    TSentryPrivilege privilege = new TSentryPrivilege(){{
  ...
  class PolicyRevokePrivilegeVerifier {
    ...
    PolicyRevokePrivilegeVerifier whenRequestStorePrivilegesReturn(Set<TSentryPrivilege> privileges)
      ...
      Mockito.when(sentryStore.listSentryPrivilegesByUsersAndGroups(
        ...
        Mockito.eq((TSentryActiveRoleSet)Mockito.any()),
        null)
